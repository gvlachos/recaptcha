...
> **Naming convention — the `app` label is the canonical application identifier, not the key display name.** `app-01-web` above is only the key's human-readable name in the console (arbitrary, for humans browsing the console — the `-web` suffix, for instance, exists purely so a future non-web key for the same app, if one is ever added, has a distinguishable name). The `app=app-01` **label** is the identifier that actually drives cost/usage attribution (Section 7) and, per Section 6, the `appId` value passed between the frontend and the shared backend. Keep the key's display name descriptive for humans; keep the label value (`app-01`, `app-02`, ...) short, stable, and consistent — that's the one identifier every other system (billing export, Looker Studio, backend config, frontend requests) should key off of.
...
```
app: app-01            # stable application identifier, one per app — this is
                        # the SAME value used as `appId` in requests between
                        # the frontend and the shared backend (Section 6.2);
                        # do not use the key's display name here.
...
5. Add an `appId` parameter (or equivalent, e.g. inferred from the caller's existing app-identifying request header) to the shared verification function so it can resolve which of the 15 site keys a given request should be checked against — this is the mechanism that lets one backend safely serve all 15 apps rather than trusting a single global site key. Use the same label-style identifier as Section 7's `app` label (e.g. `app-01`); normalize to the environment-variable-safe form inside the function, not at every call site.
